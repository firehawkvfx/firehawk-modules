...
data "aws_iam_policy_document" "cloudwatch_rendernode_logging" {
  statement {
    ...
    resources = [ # TODO: codedeploy-updater-log should be unique for the render node log, and not shared with provisioner.
      "arn:aws:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:codedeploy-rendernode-tests-log:*",
      ...
      "arn:aws:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:codedeploy-agent-log:log-stream:*"
    ]
  }
}
